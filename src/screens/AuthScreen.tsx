...
export function AuthScreen({ initialMode = "login" }: AuthScreenProps) {
  ...
  async function submit(event: FormEvent) {
    ...
    if (step === "account") navigateStep("workspace");
    else if (step === "workspace") navigateStep("runtime");
  }
  ...
  const screenMotion = reduceMotion
    ...
    : {
        initial: { opacity: 0, x: direction * 16, y: 3 },
        animate: { opacity: 1, x: 0, y: 0 },
        exit: { opacity: 0, x: direction * -12, y: -2 },
      };
  ...
  return (
    <main className="auth-entry">
      <section className="auth-art" aria-label="Cortex — distributed intelligence">
        ...
        <div className="auth-art__caption"><span>Distributed intelligence</span><strong>One operating environment.</strong></div>
      </section>

      <section className="auth-panel">
        <div className="auth-panel__atmosphere" aria-hidden />
            ...
            {mode === "login" ? (
              <motion.div key="login" className="auth-composition" {...screenMotion} transition={transition}>
                <header className="auth-heading auth-heading--login">
                  ...
                  <p>Re-enter your Cortex workspace.</p>
                </header>
                ...
                <form className="auth-form" onSubmit={submit}>
                  <LiquidSurface variant="core" className="auth-credential-rail">
                    <label className="auth-credential-row">
                      <span>Username</span>
                      <Input className="auth-rail-input" autoComplete="username" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="boss" required />
                    </label>
                    <div className="auth-rail-divider" />
                    <label className="auth-credential-row">
                      <span>Password</span>
                      <div className="auth-password">
                        <Input className="auth-rail-input" type={showPassword ? "text" : "password"} autoComplete="current-password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="••••••••••••" required />
                        <IconButton className="auth-password__toggle" size="sm" type="button" aria-label={showPassword ? "Hide password" : "Show password"} onClick={() => setShowPassword((value) => !value)}>
                          {showPassword ? <EyeOff size={15} strokeWidth={2.8} /> : <Eye size={15} strokeWidth={2.8} />}
                        </IconButton>
                      </div>
                    </label>
                  </LiquidSurface>
                  ...
                  <Button className="auth-primary" variant="primary" type="submit" loading={loginState === "loading"}>
                    ...
                    {loginState !== "loading" && <ArrowRight size={16} strokeWidth={2.8} />}
                  </Button>
                </form>

                <div className="auth-account-link">New environment? <button type="button" onClick={() => switchMode("signup")}>Create account</button></div>
              </motion.div>
            ) : (
              <motion.div key={`signup-${step}`} className="auth-composition" {...screenMotion} transition={transition}>
                <div className="auth-signup-topbar">
                  <Button className="auth-back" size="sm" variant="ghost" type="button" onClick={goBack}><ArrowLeft size={15} strokeWidth={2.8} /> Back</Button>
                  <AuthStepNav step={step} furthestStep={furthestStep} onSelect={navigateStep} />
                </div>

                {step === "account" && (
                  <>
                    <header className="auth-heading auth-heading--signup"><h1>Create your<br />account.</h1><p>Your identity is the key to one isolated Cortex environment.</p></header>
                    <form className="auth-form" onSubmit={submit}>
                      <LiquidSurface variant="core" className="auth-credential-rail">
                      ...
                      <Button className="auth-primary" variant="primary" type="submit"><span>Continue</span><ArrowRight size={16} strokeWidth={2.8} /></Button>
                    </form>
                  </>
                )}
                ...
                {step === "workspace" && (
                  <>
                    <header className="auth-heading auth-heading--signup"><h1>Name the<br />environment.</h1><p>Short, stable, operational. You can rename it later.</p></header>
                    <form className="auth-form" onSubmit={submit}>
                      <LiquidSurface variant="core" className="auth-workspace-editor">
                        <span className="auth-workspace-editor__mark"><CortexMark /></span>
                        <div><span className="auth-workspace-editor__label">Workspace</span><Input className="auth-workspace-input" value={workspaceName} onChange={(event) => setWorkspaceName(event.target.value)} required /></div>
                      </LiquidSurface>
                      <Button className="auth-primary" variant="primary" type="submit"><span>Establish workspace</span><ArrowRight size={16} strokeWidth={2.8} /></Button>
                    </form>
                  </>
                )}
                ...
                {step === "runtime" && (
                  <>
                    <header className="auth-heading auth-heading--signup auth-heading--runtime"><h1>Connect<br />Cortex.</h1><p>Attach only the infrastructure this environment actually needs.</p></header>

                    <div className="auth-runtime-stage">
                      <div className="auth-workspace-line">
                        <CortexMark />
                        <div><strong>{workspaceName || "Cortex Lab"}</strong><span>Environment established</span></div>
                      ...
                      <motion.div className="auth-connection-morph" layout transition={transition} data-layer={connectionLayer}>
                        <AnimatePresence mode="popLayout" initial={false}>
                          {connectionLayer === "closed" ? (
                            <LiquidButton layoutId="connection-surface" key="closed" variant="core" className="auth-add-connection" type="button" onClick={() => setConnectionLayer("root")} transition={transition}>
                              <Plus size={17} strokeWidth={3} /><span>Add connection</span>
                            </LiquidButton>
                          ) : (
                            <LiquidSurface layoutId="connection-surface" key="surface" variant="popover" className="auth-child-menu" transition={transition}>
                              {connectionLayer === "root" && (
                                <div className="auth-child-menu__root">
                                  <div className="auth-child-menu__head"><strong>Connection</strong><button type="button" onClick={() => setConnectionLayer("closed")} aria-label="Close connection menu"><X size={15} strokeWidth={2.8} /></button></div>
                              ...
                              {connectionLayer === "vps" && (
                                <div className="auth-child-form">
                                  <button className="auth-child-back" type="button" onClick={() => setConnectionLayer("root")}><ArrowLeft size={14} strokeWidth={2.8} /> VPS / SSH</button>
                                  <div className="auth-child-field-stack">
                                    <label className="auth-child-field"><span>Host / Tailscale IP</span><Input className="auth-child-input" value={vpsHost} onChange={(event) => setVpsHost(event.target.value)} placeholder="100.x.x.x" /></label>
                                    <label className="auth-child-field"><span>SSH user</span><Input className="auth-child-input" value={vpsUser} onChange={(event) => setVpsUser(event.target.value)} placeholder="root" /></label>
                                  </div>
                              ...
                              {connectionLayer === "api" && (
                                <div className="auth-child-form">
                                  <button className="auth-child-back" type="button" onClick={() => setConnectionLayer("root")}><ArrowLeft size={14} strokeWidth={2.8} /> API origin</button>
                                  <label className="auth-child-field"><span>Origin</span><Input className="auth-child-input" value={apiOrigin} onChange={(event) => setApiOrigin(event.target.value)} placeholder="https://api.example.com" /></label>
                                  <div className="auth-secret-note"><LockKeyhole size={13} strokeWidth={2.8} /> Secrets stay server-side.</div>
                                  <Button className="auth-child-save" variant="primary" type="button" onClick={() => setConnectionLayer("closed")}>Save connection</Button>
                                </div>
                              )}
                            </LiquidSurface>
