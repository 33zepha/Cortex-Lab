    ...
    <main className="auth-entry">
      <section className="auth-art" aria-label="Cortex — distributed intelligence">
        ...
        <div className="auth-art__caption">
          <span className="auth-art__eyebrow">Operational intelligence</span>
          <p>One environment. Multiple intelligences. Precisely coordinated.</p>
        </div>
      </section>
        ...
        <div className="auth-panel__inner">
          ...
          {mode === "login" ? (
            <>
              ...
              <button className="auth-secondary" type="button" onClick={() => switchMode("signup")}>
                Create a new account
              </button>

              <p className="auth-footnote">
                New account means a new isolated workspace and its own runtime connections.
              </p>
            </>
          ) : (
            <>
              <div className="auth-signup-topbar">
                <button className="auth-back" type="button" onClick={step === "account" ? () => switchMode("login") : goBack}>
                  <ArrowLeft size={15} />
                  Back
                </button>
              ...
              <div className="auth-progress" aria-label="Workspace setup progress">
                {steps.map((item, index) => (
                  <div key={item.id} className={`auth-progress__item ${index <= stepIndex ? "is-active" : ""}`}>
                    <span className="auth-progress__dot">{index < stepIndex ? <Check size={11} /> : index + 1}</span>
              ...
              {step === "account" && (
                <>
                  <header className="auth-heading auth-heading--signup">
                    ...
                    <p>Your identity stays separate from the infrastructure it controls.</p>
                  </header>
                  <form className="auth-form" onSubmit={submit}>
                    <label className="auth-field"><span>Email</span><input type="email" autoComplete="email" placeholder="you@domain.com" required /></label>
                    <label className="auth-field"><span>Password</span><input type="password" autoComplete="new-password" placeholder="12 characters minimum" required /></label>
              ...
              {step === "workspace" && (
                <>
                  <header className="auth-heading auth-heading--signup">
                    <p className="auth-kicker">Environment</p>
                    <h1>Establish your workspace.</h1>
                    <p>This becomes the operational boundary for missions, agents and evidence.</p>
                  </header>
                  <form className="auth-form" onSubmit={submit}>
              ...
              {step === "runtime" && (
                <>
                  <header className="auth-heading auth-heading--signup">
                    <p className="auth-kicker">Runtime connection</p>
                    <h1>Connect Cortex.</h1>
                    <p>Point this workspace to the infrastructure that will execute its missions.</p>
                  </header>
