            ...
            {mode === "login" ? (
              <motion.div key="login" className="auth-composition" {...screenMotion} transition={transition}>
                ...
                <form className="auth-form" onSubmit={submit}>
                  ...
                  <LiquidButton className="auth-action" variant="control" type="submit" disabled={loginState === "loading"}><span>{loginState === "loading" ? "Opening…" : "Enter Cortex"}</span><ArrowRight size={16} strokeWidth={2.8} /></LiquidButton>
                </form>
              ...
              <motion.div key={`signup-${step}`} className="auth-composition" {...screenMotion} transition={transition}>
                ...
                {step === "account" && <><header className="auth-heading auth-heading--signup"><h1>Create account.</h1></header><form className="auth-form" onSubmit={submit}><LiquidSurface variant="core" className="auth-credential-rail"><label className="auth-credential-row"><span>Email</span><Input className="auth-rail-input" type="email" autoComplete="email" placeholder="you@domain.com" required /></label><div className="auth-rail-divider" /><label className="auth-credential-row"><span>Password</span><Input className="auth-rail-input" type="password" autoComplete="new-password" placeholder="12 characters minimum" minLength={12} required /></label></LiquidSurface><LiquidButton className="auth-action" variant="control" type="submit"><span>Continue</span><ArrowRight size={16} strokeWidth={2.8} /></LiquidButton></form></>}
                {step === "workspace" && <><header className="auth-heading auth-heading--signup"><h1>Name your space.</h1></header><form className="auth-form" onSubmit={submit}><LiquidSurface variant="core" className="auth-workspace-editor"><span className="auth-workspace-editor__mark"><CortexMark /></span><div><span className="auth-workspace-editor__label">Workspace</span><Input aria-label="Workspace" className="auth-workspace-input" value={workspaceName} onChange={(event) => setWorkspaceName(event.target.value)} required /></div></LiquidSurface><LiquidButton className="auth-action auth-action--wide" variant="control" type="submit"><span>Establish workspace</span><ArrowRight size={16} strokeWidth={2.8} /></LiquidButton></form></>}
