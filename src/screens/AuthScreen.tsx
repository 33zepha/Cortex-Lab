...
export function AuthScreen({ initialMode = "login" }: AuthScreenProps) {
  ...
  const compactPrimary = "auth-primary !w-fit min-w-[188px] max-w-full";
          ...
          <AnimatePresence mode="wait" initial={false} custom={direction}>
            {mode === "login" ? (
              <motion.div key="login" className="auth-composition" {...screenMotion} transition={transition}>
                <header className="auth-heading auth-heading--login"><h1>Welcome<br />back.</h1></header>
                <form className="auth-form" onSubmit={submit}>
                  ...
                  <Button className={compactPrimary} variant="primary" type="submit" loading={loginState === "loading"}><span>{loginState === "loading" ? "Opening workspace…" : "Enter Cortex"}</span>{loginState !== "loading" && <ArrowRight size={16} strokeWidth={2.8} />}</Button>
                </form>
              ...
              <motion.div key={`signup-${step}`} className="auth-composition" {...screenMotion} transition={transition}>
                ...
                {step === "account" && <><header className="auth-heading auth-heading--signup"><h1>Create your<br />account.</h1></header><form className="auth-form" onSubmit={submit}><LiquidSurface variant="core" className="auth-credential-rail"><label className="auth-credential-row"><span>Email</span><Input className="auth-rail-input" type="email" autoComplete="email" placeholder="you@domain.com" required /></label><div className="auth-rail-divider" /><label className="auth-credential-row"><span>Password</span><Input className="auth-rail-input" type="password" autoComplete="new-password" placeholder="12 characters minimum" minLength={12} required /></label></LiquidSurface><Button className={compactPrimary} variant="primary" type="submit"><span>Continue</span><ArrowRight size={16} strokeWidth={2.8} /></Button></form></>}
                {step === "workspace" && <><header className="auth-heading auth-heading--signup"><h1>Name the<br />environment.</h1></header><form className="auth-form" onSubmit={submit}><LiquidSurface variant="core" className="auth-workspace-editor"><span className="auth-workspace-editor__mark"><CortexMark /></span><div><span className="auth-workspace-editor__label">Workspace</span><Input aria-label="Workspace" className="auth-workspace-input" value={workspaceName} onChange={(event) => setWorkspaceName(event.target.value)} required /></div></LiquidSurface><Button className={`${compactPrimary} min-w-[218px]`} variant="primary" type="submit"><span>Establish workspace</span><ArrowRight size={16} strokeWidth={2.8} /></Button></form></>}
