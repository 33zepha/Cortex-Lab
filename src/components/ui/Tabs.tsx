...
export function TabsList({ className, ...props }: RadixTabs.TabsListProps) {
  return (
    <RadixTabs.List
      className={cn(
        "flex items-center gap-1 overflow-x-auto scrollbar-none border-b border-border",
...
export function TabsTrigger({ className, ...props }: RadixTabs.TabsTriggerProps) {
  return (
    <RadixTabs.Trigger
      className={cn(
        "relative shrink-0 px-3 py-2.5 text-sm font-medium text-text-secondary",
        ...
        "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-border-focus rounded-t-sm",
        className,
      )}
      {...props}
    />
